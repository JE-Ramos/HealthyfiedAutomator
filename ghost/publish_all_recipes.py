...
def publish_all_remaining_recipes(batch_size=10, start_index=40):
    """Publish all remaining recipes with random delays between batches"""
    ...
    # The next date should be calculated based on previous publications
    # March 9, 2025 was the first date, and we've already published start_index recipes
    initial_date = datetime.datetime(2025, 3, 9, 12, 0, 0)
    ...
    while current_index < total_files:
        end_date = initial_date - datetime.timedelta(days=current_index)
        end_idx = min(current_index + batch_size, total_files)
        current_batch_files = recipe_files[current_index:end_idx]
        
        print(f"\n=== Processing batch {current_batch} of {num_batches} ({len(current_batch_files)} recipes) ===")
        ...
        # Random delay between batches (10-30 seconds)
        if current_index < total_files:
            delay = random.randint(10, 30)
            print(f"Waiting {delay} seconds before processing next batch...")
            time.sleep(delay)
    
    print("\nAll recipes have been published successfully!")

if __name__ == "__main__":
    # Start from where we left off (we've already published 40 recipes)
    publish_all_remaining_recipes(batch_size=10, start_index=40) 